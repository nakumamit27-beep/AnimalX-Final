...
console.log("🚀 Starting Direct Sidecar API Fetch...");
...
async function startMigration() {
  ...
  console.log(`Scanning ${data.documents.length} Animal Cards...`);
  ...
  for (const doc of data.documents) {
    ...
    if (rawPath && !rawPath.includes('cloudinary.com')) {
      ...
      console.log(`[${count}/${data.documents.length}] Fetching ID '${docId}'...`);

      let cloudUrl = null;

      // Replit sidecar endpoints
      const internalEndpoints = [
        `http://127.0.0.1:8080${cleanPath}`,
        `http://localhost:5000${cleanPath}`,
        `http://127.0.0.1:5000${cleanPath}`,
        `http://0.0.0.0:5000${cleanPath}`
      ];
  ...
  console.log("\n=============================================");
  console.log("🎉 ALL ANIMAL CARDS MIGRATION FINISHED!");
  console.log("=============================================");
}
