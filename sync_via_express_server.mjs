...
console.log("🚀 Starting Express Server Internal Route Sync...");
...
async function startMigration() {
  // Find running backend port
  let activePort = null;
  const ports = [5000, 3000, 8080, 5001];

  for (const port of ports) {
    try {
      const res = await fetch(`http://127.0.0.1:${port}/api/animals`);
      if (res.ok) {
        activePort = port;
        break;
      }
  ...
  if (!activePort) {
    activePort = 5000; // default fallback
  }

  console.log(`📡 Connected to internal Express App on Port: ${activePort}`);
  ...
  for (const doc of data.documents) {
    ...
    if (rawPath && !rawPath.includes('cloudinary.com')) {
      ...
      console.log(`[${count}/${data.documents.length}] Syncing ID '${docId}'...`);

      let cloudUrl = null;
      try {
        const imgRes = await fetch(`http://127.0.0.1:${activePort}${cleanPath}`);
        if (imgRes.ok) {
  ...
  console.log("\n=============================================");
  console.log("🎉 SUCCESS! ALL ANIMAL CARDS SYNCED TO CLOUDINARY!");
  console.log("=============================================");
}
