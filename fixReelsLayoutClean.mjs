...
function fixReels(dir) {
  ...
  for (const item of items) {
    ...
    if (fs.statSync(full).isDirectory()) {
      ...
      const cleanLayoutCode = `import React, { useState } from 'react';
...
export default function Reels() {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-black text-white relative overflow-hidden">
      ...
      <div className="w-full flex flex-col bg-black/90 border-b border-zinc-800 z-30 shrink-0">
        <div className="flex items-center justify-between px-4 py-2.5">
          <div className="flex items-center gap-2">
            <Film className="w-5 h-5 text-emerald-400" />
            <h1 className="text-base font-bold">Wildlife Reels</h1>
          </div>
          <button className="flex items-center gap-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold px-3 py-1.5 rounded-full">
            <Plus size={14} /> Upload
          </button>
        </div>
          ...
          {categories.map((cat) => (
            <button
              ...
              className={\`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap transition \${
                activeTab === cat ? 'bg-emerald-500 text-white' : 'bg-zinc-800 text-zinc-300'
              }\`}
            >
              {cat}
            </button>
      ...
      <div className="flex-1 relative w-full bg-zinc-950 overflow-hidden">
        ...
        <button
          onClick={() => setIsMuted(!isMuted)}
          className="absolute top-4 right-4 bg-black/60 p-2 rounded-full text-white z-20"
        >
        ...
        <div className="absolute bottom-6 left-4 right-16 z-20 text-white">
          ...
          <p className="text-xs text-zinc-200">{reelsData[0].title}</p>
        </div>
        ...
        <div className="absolute bottom-6 right-3 z-20 flex flex-col items-center gap-4">
          <button onClick={() => setLiked(!liked)} className="flex flex-col items-center gap-1">
            <div className="p-2.5 rounded-full bg-black/50">
              <Heart className={\`w-5 h-5 \${liked ? 'fill-red-500 text-red-500' : 'text-white'}\`} />
            </div>
            <span className="text-[10px] font-semibold">{liked ? reelsData[0].likes + 1 : reelsData[0].likes}</span>
          </button>
          ...
          <button className="flex flex-col items-center gap-1">
            ...
            <span className="text-[10px] font-semibold">{reelsData[0].comments}</span>
          </button>
          ...
          <button onClick={() => setSaved(!saved)} className="flex flex-col items-center gap-1">
            <div className="p-2.5 rounded-full bg-black/50">
              <Bookmark className={\`w-5 h-5 \${saved ? 'fill-emerald-400 text-emerald-400' : 'text-white'}\`} />
            </div>
          </button>
        </div>
      </div>
...
`;
      fs.writeFileSync(full, cleanLayoutCode, 'utf8');
      console.log(`✅ Fixed layout for: ${full}`);
    }
  }
}
