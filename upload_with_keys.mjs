...
async function startSignedUpload() {
  console.log("🚀 Starting Direct Upload to Cloudinary Media Library...");
  ...
  do {
    ...
    for (const doc of data.documents) {
      ...
      let imgVal = fields['image']?.stringValue || fields['original_image_backup']?.stringValue || '';
      ...
      if (imgVal) {
        ...
        try {
          ...
          if (cloudData.secure_url) {
            // Update Firestore with final Cloudinary URL
            ...
            console.log(`[${count}] ✅ Uploaded & Saved: ${fileName}`);
          } else {
            console.log(`[${count}] ❌ Cloudinary Error:`, cloudData.error?.message || cloudData);
          }
        } catch (err) {
  ...
  console.log("\n=============================================");
  console.log("🎉 ALL IMAGES UPLOADED TO CLOUDINARY DASHBOARD!");
  console.log("=============================================");
}
