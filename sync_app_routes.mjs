...
const APP_HOST = 'https://animal-x--Aniaml.replit.app';

console.log("🚀 Inspecting Server Storage Endpoints...");
...
async function startMigration() {
  ...
  for (const doc of data.documents) {
    ...
    if (rawPath && !rawPath.includes('cloudinary.com')) {
      ...
      // Check all possible public routes defined by your Replit Backend
      const routesToTry = [
        `${APP_HOST}${cleanPath}`,
        `${APP_HOST}/api${cleanPath}`,
        `${APP_HOST}/objects${cleanPath.replace('/api/storage/objects', '')}`
      ];
