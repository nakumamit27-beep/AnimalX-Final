...
console.log("🚀 Initializing Internal Replit Object Reader...");
...
async function startMigration() {
  ...
  for (const doc of data.documents) {
    ...
    if (rawPath && !rawPath.includes('cloudinary.com')) {
      ...
      // Access file through Express App Server storage route
      try {
        const fullUrl = `http://localhost:5000${rawPath.startsWith('/') ? '' : '/'}${rawPath}`;
      ...
      if (cloudUrl) {
        ...
        console.log(`  ⚠️ Skipped ID '${docId}': Preserved original path safe!`);
      }
    }
  }
  console.log("\n=============================================");
