...
console.log("🚀 Initializing Native Sidecar Proxy Sync...");
...
async function startMigration() {
  ...
  // Sidecar auth token from runtime
  ...
  for (const doc of data.documents) {
    ...
    if (rawPath && !rawPath.includes('cloudinary.com')) {
      ...
      console.log(`[${count}/${data.documents.length}] Syncing '${fileName}' for ID '${docId}'...`);

      let cloudUrl = null;

      // Try internal sidecar daemon ports
      const sidecarPorts = [1106, 8080, 7123];
      for (const port of sidecarPorts) {
        try {
          const targetUrl = `http://127.0.0.1:${port}/.private/uploads/${fileName}`;
