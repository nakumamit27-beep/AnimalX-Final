...
const cleanMobileReelsCode = `import React, { useState, useEffect, useRef, useCallback } from "react";
...
function scanAndRestore(dir) {
  ...
  for (const item of fs.readdirSync(dir)) {
    ...
    if (fs.statSync(full).isDirectory()) scanAndRestore(full);
    else if (item === 'Reels.jsx') {
      fs.writeFileSync(full, cleanMobileReelsCode, 'utf8');
      console.log(`✅ Clean syntax Reels.jsx written successfully at: ${full}`);
    }
  }
}
