...
const originalUploadCode = `import { useState, useRef } from "react";
...
function scan(dir) {
  ...
  for (const item of fs.readdirSync(dir)) {
    ...
    else if (item === 'UploadReel.jsx') fs.writeFileSync(full, originalUploadCode, 'utf8');
  }
}
scan('.');
console.log("✅ Step 2 Complete: UploadReel.jsx Restored with Copyright Shield!");
