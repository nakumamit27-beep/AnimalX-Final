...
- The user is asking questions about data in chat (e.g., "How many Linear issues were closed last week?") - use the `query-integration-data` skill
- The user simply wants to fetch/export/transform data without visualization - use the `query-integration-data` skill
...
The `executeSql` tool (used inside the `codeExecution` sandbox for ad hoc data queries)
should NOT be used to power runtime queries for a data-visualization
