# Consuming Animal X Design System in web apps
...
`artifacts/animal-x-ds/docs/migrating-web.md` before writing UI.
...
`node_modules/@workspace/animal-x-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/animal-x-ds/components/ui/button";
import { cn } from "@workspace/animal-x-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/animal-x-ds/hooks/use-toast";
```
...
`@workspace/animal-x-ds/components/ui/button`. Run the app's typecheck
