# Migrating Expo UI to WildSphere Design System
...
- Rewrite color/theme imports to
  `@workspace/animal-x-ds/lib/native-theme` and
  `@workspace/animal-x-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/animal-x-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/animal-x-ds` and retains only
