...
// 1. Load Local Seed Data (First 79)
let localSeed = {};
try {
...
// 2. Load Full Animals List (Extracting all values correctly)
let animalsList = [];
try {
  ...
  if (Array.isArray(rawData)) {
    ...
    for (const [cat, list] of Object.entries(rawData)) {
      if (Array.isArray(list)) {
        list.forEach(item => {
          animalsList.push(typeof item === 'string' ? item : item.name || item.baseName);
        });
      }
    }
  }
...
// Fallback to localSeed keys if list is still small
if (animalsList.length === 0) {
  animalsList = Object.keys(localSeed);
}
...
async function fetchFromAI(name) {
  const prompt = `Provide precise wildlife stats for animal "${name}" in pure valid JSON format only without codeblocks:
  {
...
async function run() {
  console.log(`🎯 Total ${animalsList.length} animals found!`);
  ...
  for (let i = 0; i < animalsList.length; i++) {
    ...
    if (!data) {
      console.log(`[${i + 1}/${animalsList.length}] ⚠️ Skipped: ${name}`);
      continue;
    }
  ...
  console.log("\n🎉 Complete! All animals populated to Firestore!");
}
