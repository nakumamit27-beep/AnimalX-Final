...
const FIRESTORE_URL = `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents/animalDetails?key=${API_KEY}`;
...
function formatValue(val) {
  ...
  if (Array.isArray(val)) {
    return { arrayValue: { values: val.map(item => formatValue(item)) } };
  }
...
async function run() {
  const dataMap = wildlifeSeedModule.default || wildlifeSeedModule.wildlifeSeed || wildlifeSeedModule;
  const entries = Object.entries(dataMap);
  console.log(`🚀 Uploading ${entries.length} Animals to Firestore...\n`);
  ...
  console.log("\n🎉 Complete! Check your app UI now.");
}
